...
==================================
Signal Stack — Layer 4: Regime Safety Score (S^safe)

Prevents "selling vol at the cliff" by gating short-vol / dispersion trades
based on the current regime environment.

Sub-components:
  (a) VIX regime penalty      — penalizes when VIX is elevated
  (b) Credit stress penalty    — penalizes widening credit spreads
  (c) Correlation regime gate  — penalizes when correlations spike (dispersion collapses)
  (d) Regime transition risk   — penalizes fast regime deterioration

  S^safe = (1 - w_vix · P_vix) × (1 - w_credit · P_credit)
           × (1 - w_corr · P_corr) × (1 - w_trans · P_trans)

When ANY component triggers hard kill → S^safe = 0.0 (absolute gate)

Integrates with existing RegimeMetrics from QuantEngine for consistency.

Ref: CBOE VIX methodology
Ref: Credit spread regime detection (Kritzman & Li, 2010)
"""
